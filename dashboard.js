...
function displayStudySessions() {
    ...
    sessions.forEach(session => {
        ...
        logEntry.textContent = `${session.date}: ${session.hoursStudied.toFixed(2)} hours`;
...
function logStudySession(duration) {
    ...
    const totalHours = Math.floor(totalStudyTimeToday / 60 * 100) / 100; // Ensure it rounds down correctly to two decimal places

    // Update the displayed total study time
    totalStudyTimeDisplay.textContent = `Total Study Time Today: ${totalHours.toFixed(2)} hours`;

    // Save the session to local storage in hours
...
function startPomodoro(duration) {
    ...
    pomodoroTimer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            // Log the exact Pomodoro duration
            logStudySession(duration); // Log the Pomodoro duration in minutes
            startBreak();
        }
    }, 1000);
}
...
function logStudySession(duration) {
    totalStudyTimeToday += duration; // Increment total study time in minutes
    const totalHours = totalStudyTimeToday / 60; // Convert to hours for display
